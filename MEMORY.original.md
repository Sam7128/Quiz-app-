...
- Archived changes: `25`
- [OS-ARC-001] `openspec/changes/archive/2026-07-16-battle-system-quality-overhaul/` (proposal, design, tasks, specs:4)
- [OS-ARC-002] `openspec/changes/archive/2026-07-20-battle-visual-upgrade/` (proposal, design, tasks, specs:7)
...
- [FACT-068] `battle-visual-upgrade` completed on 2026-07-20. Promoted 26 character actions, 12 element-phase VFX, 9 unique skill images, 5 environment overlays, and 12 typed audio cues to runtime. 7 unused items retained as source-only. Pass tsc, unit tests, lint, build, asset validator, and Playwright WebP canvas decode.
- [FACT-069] `battle-visual-upgrade` final audit v2.0 closed on 2026-07-20 with 38/38 tasks, 47 files/319 Vitest, 7/7 Chromium, Knip 0, Ponytail actionable 0, and 79 validated assets. Registry runtime-only metadata cleanup removed 251 lines; final evidence is `openspec/changes/battle-visual-upgrade/audit-report.md`.
...
- [DEC-014] Planned battle visual promotion uses only Node 22 + existing `pngjs` + Playwright Canvas. Audio paths remain registry-only (`cue-<BattleSoundCue>`), BattleArena reuses existing phase/visibility effects, and the plan forbids custom WebP parsers, second lifecycle owners, telemetry, preload/fallback managers, and generic performance frameworks.
- [DEC-015] Battle asset registry contains only runtime lookup metadata. Provenance/usage notes live in the asset manifest; UI-only presentation states do not widen `BattleAssetAction`; BGM and cues resolve from the same registry.
...
- [RG-011] `rg -n "battle-visual-upgrade|BattleSoundCue|environment" openspec/changes/battle-visual-upgrade types constants components hooks`
...
- [RISK-007] `production-source-v2` battle atlases promoted 26 actions, 12 VFX phases, 9 skill images, 5 overlays, and 12 audio cues to runtime. 7 unused items (`hero:victory`, `skeleton_wizard:cast`, `dragon_fire:fire-breath`, `environment-rubble`, `environment-ice-motes`, `environment-sparks`, `battle_victory.ogg`) remain source-only.
