...
describe('mermaidBridge', () => {
  ...
  describe('mermaidToGraph', () => {
    ...
    it('rejects graphs exceeding configured node limit', () => {
      const nodeLines = Array.from({ length: GRAPH_LIMITS.MAX_NODES + 1 }, (_, i) => `  N${i}[Node${i}]`).join('\n');
      const input = `graph TD\n${nodeLines}`;
      const result = mermaidToGraph(input);

      expect(result.success).toBe(false);
      expect(result.errors[0]).toContain(String(GRAPH_LIMITS.MAX_NODES));
    });
    ...
    it('accepts node counts above the legacy 50-node limit', () => {
      const body = Array.from({ length: 60 }, (_, index) => `N${index}[Node ${index}]`).join('\n');
      const result = mermaidToGraph(`graph TD\n${body}`);
