...
- [FACT-020] App + Quiz engine callbacks: `useCallback` to prevent race conditions.
...
- [FACT-026] Reset battle state on out-of-range bounds loaded.
...
- [FACT-035] GraphLayout & Bridge: subtree-sector radial layout with density-aware rings; sticky/image nodes preserve positions; MD parser, Nd > 12 auto-expand.
...
- [FACT-039] AI Prompts & Mermaid Import: AIPromptGuide supports Quiz + Graph tabs. GraphEditor import modal alerts syntax limits + conversion prompt copy.
- [FACT-040] KG V2: Ancestor Path + Heuristic Levenshtein ≤2 matching, no UUID in MD.
- [FACT-041] Graph images support safe external http/https URLs plus up to 4 standalone PNG/JPEG/WebP uploads; uploads are compressed to bounded WebP data URLs inside graph JSON and reuse graph cloud sync (no public Storage bucket).
...
- [FACT-045] `runHeuristicNodeMatching` extracted to `graphUtils.ts`; `useGraphCodeMode.ts` shrunk to 104 lines; all `: any` removed from challenger tests (NO_ANY).
- [FACT-046] `graphCloudStorage.ts` LWW sync + `mindspark_dirty_graphs` queue; autosave upload + 2-layer ConfirmDialog conflict resolution (Keep Local / Use Cloud / Save Copy); online auto-retry; beta gate removed — KG is stable feature.
...
- [FACT-049] Six branch-coherent presets via `constants/graphThemes.ts` + `utils/graphColorHelper.ts`; sticky/image nodes are preserved; radial moves concept nodes only; solid bg uses `${color}CC`.
...
- [FACT-053] 5 graph capability specs synced to `openspec/specs/`. V1 audit artifacts moved to `docs/audits/knowledge-graph-v2-upgrade/`. V2 report kept in change archive.
- [FACT-054] Canonical auto-layout API: `applyAutoLayout`. Deprecated `applyDagreLayout` alias (no dead direction param). `useGraphConflictResolver` in `useGraphStorage`. `setCodeErrors` dead export removed. ConceptNode shape guard.
- [FACT-055] Graph cloud fallback: `PGRST205`/missing `knowledge_graphs` disables cloud sync for the current page session, preserves local graphs, and deduplicates same-user in-flight sync requests.
- [FACT-056] Node handle click opens `NodeQuickMenu` (edit, 8 shapes, add child, delete); drag-to-blank still uses `DropNodeMenu`.
- [FACT-057] Free layout permits concept dragging; radial layout applies subtree-aware auto-placement and locks concepts while keeping sticky/image draggable.
- [FACT-058] Self-loop/dangling graph edges are removed during normalization after one-time raw backup at `mindspark_graphs_backup_pre_v3_cleanup`.
- [FACT-059] Supabase JS 2.110.5 is required for Auth Web Locks orphan-lock recovery; project Node engine is >=22. Custom `runWithSyncLock` returns the native lock Promise so AbortError is observable.
- [FACT-060] Knowledge Graph shapes: hexagon uses an SVG polygon with a complete stroke; cloud uses an SVG path with multiple lobes. Progressive reading cycles through L1/L2/L3 via `hooks/graphStateUtils.ts`.
- [FACT-061] Knowledge Graph progressive reading is branch-based: `hooks/graphStateUtils.ts` exposes root-plus-direct-child visibility and per-node branch toggling; `GraphEditor.tsx` filters hidden descendant nodes and their edges while preserving the full graph in state.
...
- [DEC-005] Build: `vite build`. Prevent Windows path permission issues.
...
- [DEC-009] Autosave cloud: `uploadGraphToCloudSafely`, never silently overwrite newer cloud timestamps.
- [DEC-010] `applyDagreLayout` kept as deprecated alias until 2026-10-01; new callers use `applyAutoLayout`.
- [DEC-011] Supabase migration `20260714000000_create_knowledge_graphs.sql` must be applied remotely to restore graph cloud sync; client local-only fallback remains required for stale schema-cache environments.
- [DEC-012] Uploaded graph images stay private to graph JSON/offline storage and existing graph sync; do not introduce a public Supabase Storage bucket without a new privacy/capacity design.
...
- [RISK-004] Playwright CLI knowledge-graph suite can hang in local runner; Node/Playwright smoke path passes.
- [RISK-005] knip reports pre-existing unused files/exports + unlisted dependency outside this change.
- [RISK-006] Current Supabase project has not exposed `public.knowledge_graphs` in schema cache; cloud graph sync remains local-only until migration deployment is verified.
