...
import { ChunkedPracticePanel } from './ChunkedPracticePanel';
...
interface DashboardProps {
  ...
  chunkedPractice?: {
    ...
    onCreateSession: (chunkSize: number) => void;
    ...
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between bg-white/80 dark:bg-slate-900/60 backdrop-blur-xl rounded-3xl p-8 shadow-sm border border-white/20 dark:border-white/5 relative overflow-hidden group">
        ...
        <div className="mt-6 md:mt-0 flex flex-wrap gap-4 items-center relative z-10">
          ...
          {chunkedPractice && (
            <button
              onClick={() => setShowChunkPanel(prev => !prev)}
              disabled={totalQuestions === 0}
              className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white px-6 py-3.5 rounded-2xl font-bold disabled:opacity-50"
            >
              分階段練習
            </button>
          )}
      ...
      {chunkedPractice && showChunkPanel && (
        <ChunkedPracticePanel
          selectedBankCount={selectedBankIds.length}
          totalQuestions={totalQuestions}
          chunkSizeOptions={chunkedPractice.chunkSizeOptions}
          isCreating={chunkedPractice.isCreatingSession}
          onCreate={(chunkSize) => chunkedPractice.onCreateSession(chunkSize)}
          onClose={() => setShowChunkPanel(false)}
        />
      )}

      {chunkedPractice && chunkedPractice.activeSessions.length > 0 && (
        <div className="bg-white/80 dark:bg-slate-900/60 backdrop-blur-xl p-6 rounded-3xl border border-white/20 dark:border-white/5">
          <h3 className="text-slate-800 dark:text-white font-bold text-lg mb-4">分階段練習接力</h3>
          <div className="grid gap-3 md:grid-cols-2">
            {chunkedPractice.activeSessions.map((session) => (
              <ActiveSessionCard
                key={session.id}
                session={session}
                onContinue={chunkedPractice.onContinueSession}
                onAbandon={chunkedPractice.onAbandonSession}
              />
            ))}
          </div>
        </div>
